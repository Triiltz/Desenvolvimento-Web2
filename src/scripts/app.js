...
class FuelStationApp {
    ...
    loadMockData() {
        // Mock fuel stations data - in a real app this would come from an API
        this.stations = [
            {
                id: 1,
            ...
            },
            {
                id: 2,
                name: "Shell Select",
                address: "R. Dr. João Batista de Lacerda, 150 - Centro",
                lat: -22.0185,
                lng: -47.8920,
                rating: 4.2,
                distance: "280m",
                fuels: {
                    gasoline: { price: 6.05, updated: "há 1h" },
                    ethanol: { price: 4.35, updated: "há 1h" },
                    diesel: { price: 5.55, updated: "há 2h" }
                },
                features: ["Conveniência", "Café", "Wi-Fi"]
            },
            {
                id: 3,
                name: "Ipiranga Express",
                address: "Av. Getúlio Vargas, 890 - Vila Prado",
                lat: -22.0210,
                lng: -47.8890,
                rating: 4.8,
                distance: "320m",
                fuels: {
                    gasoline: { price: 5.89, updated: "há 30min" },
                    ethanol: { price: 4.19, updated: "há 30min" },
            ...
            },
            {
                id: 4,
                name: "Posto Ale",
                address: "R. Episcopal, 1570 - Centro",
                lat: -22.0175,
                lng: -47.8935,
                rating: 4.0,
                distance: "450m",
                fuels: {
                    gasoline: { price: 6.10, updated: "há 4h" },
                    ethanol: { price: 4.40, updated: "há 4h" },
                    diesel: { price: 5.60, updated: "há 5h" }
                },
            ...
            },
            {
                ...
                address: "Av. São Carlos, 2100 - Jardim Paulista",
                lat: -22.0230,
                lng: -47.8870,
                rating: 4.3,
                distance: "650m",
                fuels: {
                    gasoline: { price: 5.95, updated: "há 1h" },
                    ethanol: { price: 4.25, updated: "há 1h" },
                    diesel: { price: 5.45, updated: "há 2h" }
                },
    ...
    createCustomIcon(station) {
        return L.divIcon({
            ...
            html: `<div class="custom-marker" style="background: ${this.getStationColor(station.rating)}"></div>`,
            iconSize: [30, 30],
            iconAnchor: [15, 30]
        });
    }
        ...
        return `
            <div class="popup-station">
                ...
                <div class="popup-fuel-preview">
                    <div class="popup-fuel-item">G: R$${cheapestFuel.gasoline}</div>
                    <div class="popup-fuel-item">E: R$${cheapestFuel.ethanol}</div>
                    <div class="popup-fuel-item">D: R$${cheapestFuel.diesel}</div>
                </div>
                <button class="popup-view-more" onclick="app.showStationModal(${JSON.stringify(station).replace(/"/g, '&quot;')})">
                    Ver detalhes
                </button>
            </div>
        `;
    ...
    updateMapWithUserLocation() {
        if (this.userLocation) {
            ...
            this.userMarker.bindPopup('<div style="text-align: center; padding: 8px;"><strong>Você está aqui!</strong></div>');
    ...
    showStationModal(station) {
        ...
        document.getElementById('modalDistance').textContent = station.distance + ' de você';
        
        // Update rating
        const starsContainer = document.getElementById('modalStars');
        starsContainer.innerHTML = this.generateStarsHTML(station.rating, 20);
        
        document.getElementById('modalRating').textContent = `${station.rating} de 5`;
        ...
        return fuelTypes.map(fuel => {
            ...
            return `
                <div class="fuel-item">
                    ...
                    <div class="fuel-price">R$ ${fuelData.price.toFixed(2)}</div>
                    <div class="fuel-updated">Atualizado ${fuelData.updated}</div>
                </div>
            `;
        }).join('');
    }
    ...
    handleSearch(query) {
        ...
        // For now, we'll just filter existing stations
        ...
        if (filtered.length > 0) {
            ...
            this.map.setView([station.lat, station.lng], 17);
            // Trigger click on marker
            const markerData = this.markers.find(m => m.station.id === station.id);
            if (markerData) {
